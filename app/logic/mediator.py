...
class Mediator:
    ...
    async def handle_event(self, event: BaseEvent) -> Iterable[ER]:
        ...
        if not handlers:
            raise EventHandlersNotRegisterException(event_type)
        
        return [await handler.handle(event) for handler in handlers]
    
    async def handle_commands(self, command: BaseCommand) -> Iterable[CR]:
        command_type = command.__class__
        handlers = self.events_map.get(command_type)

        if not handlers:
            raise CommandHandlersNotRegisterException(command_type)
